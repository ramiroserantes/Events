    ...
    public StringBuilder joinMission(UUID playerId, EventsPlayer eventsPlayer, Guild guild, Long missionId) {
        ...
        try (Connection connection = dataSource.getConnection()) {
            ...
            for(ObjectiveData obj : missionObjs) {
                ...
                if(objective == null) {
                    objective = new Objective(missionId, missionData.getExpiration(), missionData.getGrade(), obj
                    .getTarget(), obj.getLevels()
                            , obj.getReqAmount());
    ...
    public Penalty initPlayer(UUID playerId) {
        ...
        try (Connection connection = dataSource.getConnection()) {
            ...
            for (MissionData ms : missionDataList) {
                ...
                objectiveDataList = missionDao.findMissionCurrentObjectivesByPlayer(playerId, ms.getMissionId(),
                connection);
